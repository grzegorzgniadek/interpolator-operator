...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name interpolator-operator-builder
	$(CONTAINER_TOOL) buildx use interpolator-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm interpolator-operator-builder
	rm Dockerfile.cross
...
.PHONY: helm-docs
helm-docs-install: $(HELMDOCS) ##Install latest helm-docs binary
$(HELMDOCS): $(LOCALBIN) 
...
helm-docs-generate: helm-docs-install ## Generate Charts README.md
...
HELM_NAMESPACE ?= interpolator-operator-system
...
.PHONY: helm-deploy
helm-deploy: install-helm generate-helm ## Deploy manager to the K8s cluster via Helm. Specify an image with IMG.
